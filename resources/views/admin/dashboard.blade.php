...
@section('title', 'Bảng Điều Khiển Quản Trị - Dong Anh Food Map')
            ...
            <div id="adminTab-overview" class="admin-tab-content">
                ...
                <div class="stats-grid" style="margin-bottom: 30px;">
                    <div class="stat-card glass-panel">
                        <div>
                            <div class="stat-value">{{ $stats['total_eateries'] }}</div>
                    ...
                    <div class="stat-card glass-panel">
                        <div>
                            <div class="stat-value">{{ $stats['total_communes'] }}</div>
                            ...
                            <table class="admin-table">
                                ...
                                <tbody>
                                    @foreach($eateries as $eat)
                                        <tr>
                                            <td>
                                                <img src="{{ $eat->image_path ?: 'https://images.unsplash.com/photo-1591814468924-caf88d1232e1?auto=format&fit=crop&w=300&q=80' }}" style="width: 54px; height: 54px; object-fit: cover; border-radius: 8px; border: 1px solid var(--border-glow);" alt="{{ $eat->name }}">
                                            </td>
                                            <td>
                                                <strong style="color: var(--text-main); display: block;">{{ $eat->name }}</strong>
                                                <span style="font-size: 0.75rem; color: var(--text-muted); display: block; max-width: 220px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">📍 {{ $eat->address }}</span>
                                            </td>
                                            <td>
                                                <span class="tag-badge-accent" style="font-size: 0.7rem; padding: 2px 8px;">{{ $eat->category->icon }} {{ $eat->category->name }}</span>
                                            </td>
                                            <td>
                                                <span style="font-size: 0.9rem;">Xã {{ $eat->commune->name }}</span>
                                            </td>
                                            <td>
                                                <span style="font-size: 0.9rem; font-family: var(--font-heading); color: var(--primary);">{{ $eat->phone ?: 'Chưa có' }}</span>
                                            </td>
                                            <td>
                                                <div style="font-size: 0.85rem; display: flex; flex-direction: column; gap: 2px;">
                                                    <span style="color: #ffc107; font-weight: 700;">★ {{ number_format($eat->average_rating, 1) }}</span>
                                                    <span style="font-size: 0.7rem; color: var(--text-muted);">({{ $eat->reviews->count() }} reviews)</span>
                                                </div>
                                            </td>
                                            <td style="text-align: center;">
                                                <div style="display: inline-flex; gap: 8px;">
                                                    <a href="/dia-diem/dac-san/{{ $eat->slug }}" target="_blank" class="btn-secondary" style="padding: 6px 12px; font-size: 0.75rem; border-radius: 6px;">
                                                        Xem
                                                    </a>
                                                    <a href="/admin/eateries/{{ $eat->id }}/edit" class="btn-accent" style="padding: 6px 12px; font-size: 0.75rem; border-radius: 6px;">
                                                        Sửa
                                                    </a>
            ...
            <div id="adminTab-videos" class="admin-tab-content" style="display: none;">
                ...
                <div class="stats-grid" style="margin-bottom: 25px;">
                    ...
                    <div class="stat-card glass-panel" style="border-left: 3px solid #28a745;">
                        <span class="stat-icon">✅</span>
                        <div class="stat-info">
                            <span class="stat-value">{{ $videos->where('status', 'approved')->count() }}</span>
                    ...
                    <div class="stat-card glass-panel" style="border-left: 3px solid #ffc107;">
                        ...
                        <div class="stat-info">
                            <span class="stat-value">{{ $videos->where('status', 'pending')->count() }}</span>
                ...
                <div class="split-screen" style="display: flex; gap: 24px; flex-wrap: wrap;">
                    ...
                    <div class="glass-panel" style="flex: 1; min-width: 320px; padding: 22px; display: flex; flex-direction: column; gap: 20px; max-width: 380px; height: fit-content;">
                        ...
                        <form action="{{ route('admin.video.store') }}" method="POST" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 16px;">
                            @csrf
                            ...
                            <div class="review-form-group">
                                ...
                                <select name="eatery_id" required class="form-input" style="font-size: 0.85rem;">
                                    ...
                                    @foreach($eateries as $eat)
                                        <option value="{{ $eat->id }}">{{ $eat->name }} ({{ $eat->commune->name }})</option>
                                    @endforeach
                                </select>
                            </div>
                                    ...
                                    <tbody>
                                        @foreach($videos as $vid)
                                            <tr>
                                                <td>
                                                    <div style="display: flex; align-items: center; gap: 10px;">
                                                        <div style="position: relative; width: 45px; height: 60px; border-radius: 6px; overflow: hidden; background: #000; flex-shrink: 0; border: 1px solid var(--border-glow);">
                                                            <img src="{{ $vid->thumbnail_path ?: 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?auto=format&fit=crop&w=100&q=80' }}" style="width: 100%; height: 100%; object-fit: cover; opacity: 0.8;">
                                                ...
                                                <td>
                                                    <div style="display: flex; flex-direction: column;">
                                                        <span style="font-weight: 600; font-size: 0.82rem; color: var(--text-main);">{{ $vid->eatery->name }}</span>
                                                        <span style="font-size: 0.72rem; color: var(--primary);">{{ $vid->eatery->category->name }}</span>
                                                    </div>
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    <span style="font-size: 0.82rem; font-weight: 700; color: #e83e8c;">❤️ {{ number_format($vid->likes_count) }}</span>
                                                </td>
                                                <td>
                                                ...
                                                <td style="text-align: center;">
                                                    <div style="display: inline-flex; gap: 8px;">
                                                        @if(session('user_role') === 'admin' && $vid->status === 'pending')
                                                            ...
                                                            <form action="{{ route('admin.video.reject', $vid->id) }}" method="POST" style="display: inline;">
                                                                @csrf
                                                        ...
                                                        @if(session('user_role') === 'admin' || $vid->user_id === session('user_id') || $vid->eatery->user_id === session('user_id'))
                                                            <button type="button" class="btn-accent" onclick="openEditVideoModal('{{ $vid->id }}', '{{ addslashes($vid->title) }}', '{{ $vid->eatery_id }}', '{{ $vid->video_url }}', '{{ $vid->video_type }}')" style="padding: 6px 12px; font-size: 0.75rem; border-radius: 6px; background: linear-gradient(135deg, #20b2aa, #17a2b8); border: none; color: #fff; font-weight: 700;">
                                                                Sửa
                                                            </button>
                                                        @endif
