...
<script>
            ...
            const popupContent = `
                <div style="width: 100%; font-family: inherit; position: relative;">
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 4px; padding-top: 6px; border-top: 1px solid rgba(226, 232, 240, 0.5);">
                        ...
                        <a href="/dia-diem/dac-san/${stop.slug}" style="padding: 5px 12px; border-radius: 20px; background: ${isCooking ? 'linear-gradient(135deg, #10b981 0%, #059669 100%)' : 'linear-gradient(135deg, #ff7e29 0%, #ff5e00 100%)'}; color: #ffffff; text-decoration: none; font-weight: 800; font-size: 0.68rem; box-shadow: 0 3px 8px ${isCooking ? 'rgba(16, 185, 129, 0.2)' : 'rgba(255, 126, 41, 0.2)'}; transition: all 0.2s;">
                            Xem chi tiết
                        </a>
                    </div>
                </div>
        ...
        if (target < 0) {
            ...
            if (completedStops.size === stopsData.length) {
                ...
                const endMsg = tourMood === 'cooking'
                    ? "🏁 Bạn đã đến điểm cuối của cuộc hành trình! Bạn có muốn kết thúc Trải nghiệm & Tự nấu và xem Chứng Nhận Tổng Kết Nhật Ký chuyến đi ngay không?"
    ...
    function triggerCheckIn(event, index) {
        ...
        if (!isJourneyMode) {
            const confirmMsg = tourMood === 'cooking'
                ? "🚀 Bạn cần kích hoạt hành trình 'Bắt đầu Trải nghiệm & Tự nấu' để ghi nhận GPS đi đường và thực hiện Check-in chặng này. Bắt đầu ngay nhé?"
    ...
    function shootConfetti() {
        ...
        const colors = ['#ff7e29', '#ffb03a', '#10b981', '#3b82f6', '#ec4899', '#a855f7'];
...
<div id="reviewModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(15, 23, 42, 0.2); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); z-index: 10000; align-items: center; justify-content: center;">
    <div style="width: 90%; max-width: 420px; padding: 26px; border-radius: 24px; background: rgba(255, 255, 255, 0.9); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 1.5px solid rgba(255,255,255,0.6); box-shadow: 0 20px 40px rgba(15, 23, 42, 0.08); color: #0f172a; position: relative;">
        ...
        <div style="margin-bottom: 16px; text-align: center;">
            ...
            <span id="starRatingLabel" style="display: block; font-size: 0.78rem; font-weight: 700; color: #ff7e29; margin-top: 4px;">Tuyệt vời! (5/5)</span>
        </div>
...
<div id="completionModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(15, 23, 42, 0.2); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); z-index: 10001; align-items: center; justify-content: center; padding: 15px 20px;">
    <div class="completion-scroll-card" style="width: 100%; max-width: 510px; padding: 22px 26px; border-radius: 24px; background: rgba(255, 255, 255, 0.96); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 2px solid rgba(255, 255, 255, 0.9); position: relative; box-shadow: 0 25px 60px rgba(15, 23, 42, 0.15); color: #0f172a; overflow: hidden;">
        <button onclick="closeCompletionModal()" style="position: absolute; top: 16px; right: 16px; background: none; border: none; font-size: 1.2rem; color: #64748b; cursor: pointer; transition: color 0.2s;" onmouseover="this.style.color='#ff7e29'" onmouseout="this.style.color='#64748b'">✕</button>
        ...
        <div id="certificateTicket" style="background: rgba(248, 250, 252, 0.7); border: 1.5px dashed var(--primary); border-radius: 14px; padding: 12px; margin-bottom: 12px;">
            ...
            <div style="margin-bottom: 8px; text-align: center;">
                ...
                <span id="overallRatingLabel" style="display: block; font-size: 0.72rem; font-weight: 700; color: #ff7e29; margin-top: 1px;">Tuyệt vời! (5/5)</span>
            </div>
...
<div id="authGateModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(15, 23, 42, 0.4); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); z-index: 10002; align-items: center; justify-content: center; animation: fadeIn 0.3s ease;">
    <div style="width: 90%; max-width: 440px; padding: 32px 28px; border-radius: 28px; background: rgba(26, 26, 38, 0.7); backdrop-filter: blur(25px); -webkit-backdrop-filter: blur(25px); border: 1.5px solid rgba(255, 126, 41, 0.25); box-shadow: 0 20px 50px rgba(0, 0, 0, 0.3), 0 0 30px rgba(255, 126, 41, 0.15); color: #ffffff; position: relative; text-align: center;">
        ...
        <div style="margin-bottom: 24px; position: relative; display: inline-block;">
            <div style="width: 76px; height: 76px; border-radius: 50%; background: rgba(255, 126, 41, 0.15); border: 2.5px solid var(--primary); display: flex; align-items: center; justify-content: center; margin: 0 auto; box-shadow: 0 0 20px rgba(255, 126, 41, 0.3); animation: float 3s ease-in-out infinite;">
                <span style="font-size: 2.5rem; filter: drop-shadow(0 0 8px rgba(255, 126, 41, 0.6));">✨</span>
            </div>
        </div>
        
        <span style="background: rgba(255,126,41,0.15); color: var(--primary); padding: 4px 14px; border-radius: 20px; font-size: 0.68rem; font-weight: 800; letter-spacing: 1.2px; text-transform: uppercase; display: inline-block; margin-bottom: 12px; border: 1px solid rgba(255,126,41,0.2);">
        ...
        <div style="display: flex; flex-direction: column; gap: 12px; margin-bottom: 16px;">
            ...
            <a id="authGateRegisterBtn" href="#" style="padding: 13px; border-radius: 14px; font-weight: 800; font-size: 0.88rem; text-decoration: none; color: #ffffff; background: linear-gradient(135deg, #ff7e29 0%, #ff5e00 100%); border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 6px; box-shadow: 0 0 20px rgba(255, 126, 41, 0.35); transition: all 0.3s;" onmouseover="this.style.transform='scale(1.02)'; this.style.boxShadow='0 0 25px rgba(255, 126, 41, 0.5)';" onmouseout="this.style.transform='none'; this.style.boxShadow='0 0 20px rgba(255, 126, 41, 0.35)';">
...
<div id="customAlertModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(15, 23, 42, 0.45); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); z-index: 100010; align-items: center; justify-content: center; animation: fadeIn 0.3s ease;">
    <div style="width: 85%; max-width: 360px; padding: 26px 24px; border-radius: 24px; background: rgba(255, 255, 255, 0.98); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 1.5px solid rgba(255, 255, 255, 0.9); box-shadow: 0 25px 50px rgba(0, 0, 0, 0.15), 0 0 0 1px rgba(0,0,0,0.05); text-align: center; position: relative;">
        ...
        <div style="width: 60px; height: 60px; border-radius: 50%; background: rgba(255, 126, 41, 0.12); display: flex; align-items: center; justify-content: center; margin: 0 auto 16px auto; animation: bounce 2s infinite;">
        ...
        <button onclick="closeCustomAlert()" class="btn-primary" style="display: inline-flex; align-items: center; justify-content: center; padding: 10px 28px; border-radius: 50px; font-weight: 700; font-size: 0.88rem; border: none; cursor: pointer; background: linear-gradient(135deg, #ff7e29 0%, #ff5e00 100%); color: #ffffff; box-shadow: 0 6px 16px rgba(255, 126, 41, 0.3); transition: all 0.3s ease;" onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 8px 22px rgba(255, 126, 41, 0.45)';" onmouseout="this.style.transform='none'; this.style.boxShadow='0 6px 16px rgba(255, 126, 41, 0.3)';">
