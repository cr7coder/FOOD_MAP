<?php $__env->startSection('title', '👁️ Chi tiết User: ' . $user->name); ?>
...
<div class="admin-welcome-banner" style="margin-bottom: 24px;">
    <div style="display: flex; justify-content: space-between; align-items: center; width: 100%;">
        <div>
            ...
            <p>Xem thông tin chi tiết hồ sơ tài khoản #<?php echo e($user->id); ?></p>
        </div>
...
<div class="admin-card" style="max-width: 850px; margin: 0 auto; box-shadow: 0 10px 30px -5px rgba(0, 0, 0, 0.08); border-radius: 20px; overflow: hidden; padding: 0; border: none;">
    ...
    <div style="background: linear-gradient(135deg, #7c3aed 0%, #6d28d9 100%); padding: 32px 32px 100px 32px; position: relative;">
        <div style="display: flex; gap: 20px; align-items: center;">
            ...
            <div>
                ...
                <span style="color: rgba(255, 255, 255, 0.85); font-size: 0.86rem; margin-top: 4px; display: block;">✉️ <?php echo e($user->email); ?></span>
            </div>
        </div>
    </div>
    ...
    <div style="padding: 0 32px; margin-top: -60px; position: relative; z-index: 10; display: grid; grid-template-columns: 1fr; gap: 16px; margin-bottom: 32px; max-width: 300px;">
        ...
        <div style="padding: 16px 20px; background-color: #ffffff; border-radius: 12px; border: 1px solid rgba(124, 58, 237, 0.15); box-shadow: 0 10px 15px -3px rgba(124, 58, 237, 0.05); display: flex; flex-direction: column;">
            ...
            <strong style="font-size: 1.1rem; color: var(--admin-text-main);"><?php echo e($user->created_at->format('d/m/Y')); ?></strong>
        </div>
    </div>
    ...
    <div style="padding: 0 32px 32px 32px;">
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-bottom: 32px;">
            ...
            <div style="padding: 14px 20px; border: 1.5px solid var(--admin-border); border-radius: 12px; background-color: #f8fafc;">
                ...
                <strong style="font-size: 0.95rem; color: var(--admin-text-main);"><?php echo e($user->phone ?: 'Chưa cập nhật'); ?></strong>
            </div>
            ...
            <div style="padding: 14px 20px; border: 1.5px solid var(--admin-border); border-radius: 12px; background-color: #f8fafc;">
                ...
                <strong style="font-size: 0.95rem; color: var(--admin-text-main);"><?php echo e($user->email); ?></strong>
            </div>
            ...
            <div style="padding: 14px 20px; border: 1.5px solid var(--admin-border); border-radius: 12px; background-color: #f8fafc;">
                ...
                <strong style="font-size: 0.95rem; color: var(--admin-text-main);"><?php echo e($user->updated_at->format('H:i d/m/Y')); ?></strong>
            </div>
        ...
        <div style="display: flex; justify-content: flex-end; gap: 12px; border-top: 1.5px solid var(--admin-border); padding-top: 24px;">
            <?php if($user->id !== session('user_id')): ?>
                <!-- Toggle status form -->
                <form action="/admin/users/<?php echo e($user->id); ?>/toggle-status" method="POST" onsubmit="showCustomConfirm(event, this, '<?php echo e($user->status === 'active' ? 'Vô hiệu hóa tài khoản' : 'Kích hoạt tài khoản'); ?>', 'Bạn có chắc chắn muốn <?php echo e($user->status === 'active' ? 'vô hiệu hóa' : 'kích hoạt lại'); ?> tài khoản của người dùng <?php echo e($user->name); ?>?', false)" style="display: inline; margin: 0;">
                    <?php echo csrf_field(); ?>
                    <button type="submit" class="btn-admin" style="padding: 10px 24px; border-radius: 10px; font-weight: 700; font-size: 0.82rem; background-color: #d97706; color: #ffffff; border: none; display: flex; align-items: center; gap: 6px; cursor: pointer;">
                        🔌 <?php echo e($user->status === 'active' ? 'Vô hiệu hóa' : 'Kích hoạt'); ?>
            ...
            <a href="/admin/users/<?php echo e($user->id); ?>/edit" class="btn-admin btn-admin-primary" style="padding: 10px 28px; border-radius: 10px; font-weight: 700; font-size: 0.82rem; text-decoration: none; display: flex; align-items: center; gap: 6px;">
                ✏️ Chỉnh sửa
            </a>

            <?php if($user->id !== session('user_id')): ?>
                <!-- Delete user form -->
                <form action="/admin/users/<?php echo e($user->id); ?>" method="POST" onsubmit="showCustomConfirm(event, this, 'Xóa tài khoản', 'Bạn có chắc chắn muốn xóa vĩnh viễn người dùng <?php echo e($user->name); ?> khỏi hệ thống? Hành động này không thể hoàn tác!', true)" style="display: inline; margin: 0;">
                    <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\FOODDA\FOOD_MAP\resources\views/admin/users/show.blade.php ENDPATH**/ ?>