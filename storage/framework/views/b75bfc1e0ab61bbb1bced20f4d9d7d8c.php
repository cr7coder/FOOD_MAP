<?php $__env->startSection('title', '⚙️ Chỉnh sửa User: ' . $user->name); ?>
...
<div class="admin-welcome-banner" style="margin-bottom: 24px;">
    <div style="display: flex; justify-content: space-between; align-items: center; width: 100%;">
        <div>
            ...
            <p>Thay đổi thông tin và quyền hạn của thành viên #<?php echo e($user->id); ?></p>
        </div>
...
<div class="admin-card" style="max-width: 800px; margin: 0 auto; box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.05); border-radius: 20px;">
    <div class="admin-card-header" style="border-bottom: 1.5px solid var(--admin-border); padding-bottom: 16px;">
        <h2 class="admin-card-title" style="font-size: 1.1rem; font-weight: 700; color: var(--admin-primary); display: flex; align-items: center; gap: 8px;">
            <span>✏️</span> Chỉnh Sửa User: <?php echo e($user->name); ?>

        </h2>
    </div>

    <form action="/admin/users/<?php echo e($user->id); ?>" method="POST" style="padding: 24px 0 8px 0;">
        <?php echo csrf_field(); ?>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
            ...
            <div class="admin-form-group">
                ...
                <div style="position: relative;">
                    ...
                    <input type="text" name="name" required value="<?php echo e(old('name', $user->name)); ?>" class="admin-form-input" placeholder="Nhập họ và tên..." style="padding-left: 38px; border-radius: 10px; height: 42px;">
                </div>
            </div>
            ...
            <div class="admin-form-group">
                ...
                <div style="position: relative;">
                    <span style="position: absolute; left: 14px; top: 50%; transform: translateY(-50%); color: var(--admin-text-muted);">👤</span>
                    <input type="text" name="avatar" required value="<?php echo e(old('avatar', $user->avatar ?: '🧑')); ?>" class="admin-form-input" placeholder="Nhập Emoji đại diện..." style="padding-left: 38px; border-radius: 10px; height: 42px;">
                </div>
            </div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
            ...
            <div class="admin-form-group">
                ...
                <div style="position: relative;">
                    ...
                    <input type="email" name="email" required value="<?php echo e(old('email', $user->email)); ?>" class="admin-form-input" placeholder="email@example.com" style="padding-left: 38px; border-radius: 10px; height: 42px;">
                </div>
            </div>
            ...
            <div class="admin-form-group">
                ...
                <div style="position: relative;">
                    <span style="position: absolute; left: 14px; top: 50%; transform: translateY(-50%); color: var(--admin-text-muted);">📞</span>
                    <input type="text" name="phone" required value="<?php echo e(old('phone', $user->phone)); ?>" class="admin-form-input" placeholder="0901234567" style="padding-left: 38px; border-radius: 10px; height: 42px;">
                </div>
            </div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
            ...
            <div class="admin-form-group">
                ...
                <div style="position: relative;">
                    ...
                    <select name="role" required class="admin-form-input" style="padding-left: 38px; border-radius: 10px; height: 42px; font-size: 0.86rem;">
                        <option value="user" <?php echo e(old('role', $user->role) === 'user' ? 'selected' : ''); ?>>Customer (Khách hàng)</option>
                        <option value="seller" <?php echo e(old('role', $user->role) === 'seller' ? 'selected' : ''); ?>>Seller (Chủ cơ sở ẩm thực)</option>
                        <option value="admin" <?php echo e(old('role', $user->role) === 'admin' ? 'selected' : ''); ?>>Admin (Quản trị viên hệ thống)</option>
                    </select>
                </div>
            </div>
            ...
            <div class="admin-form-group">
                ...
                <div style="position: relative;">
                    <span style="position: absolute; left: 14px; top: 50%; transform: translateY(-50%); color: var(--admin-text-muted);">🔌</span>
                    <select name="status" required class="admin-form-input" style="padding-left: 38px; border-radius: 10px; height: 42px; font-size: 0.86rem;" <?php echo e($user->id === session('user_id') ? 'disabled' : ''); ?>>
                        <option value="active" <?php echo e(old('status', $user->status) === 'active' ? 'selected' : ''); ?>>Hoạt động (Active)</option>
                        <option value="disabled" <?php echo e(old('status', $user->status) === 'disabled' ? 'selected' : ''); ?>>Vô hiệu hóa (Disabled)</option>
                    </select>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\FOODDA\FOOD_MAP\resources\views/admin/users/edit.blade.php ENDPATH**/ ?>